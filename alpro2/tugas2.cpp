...
structBarang barang[maxEntryData+1], barangSorted[maxEntryData+1]; // +1 karena array yg diisi tidak dari 0. REVIEW: model karo view ojo samakan, start ko 0?
...
void input_isiKode(int index){
	cout << "1. Kode \t\t: "; gets(barang[index].kode);
}
...
void sorting(){
	...
	for(int i=1; i<index; i++){
		for(int j=1; j<index; j++){
			if(barangSorted[j].kode > barangSorted[j+1].kode){
				temporer = barangSorted[j+1];
				barangSorted[j+1] = barangSorted[j];
				barangSorted[j] = temporer;
			}
		}
	}
}
...
int search_formulir(){
	...
	for(int i = 1; i <= maxEntryData; i++){
		const char * str1 = barang[i].kode;
		const char * str2 = kode;
